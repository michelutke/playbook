...
- [x] **T-001** Create `attendance_responses` table
...
- [x] **T-002** Create `attendance_records` table
...
- [x] **T-003** Create `abwesenheit_rules` table + `abwesenheit_backfill_jobs` table
...
- [x] **T-005** `GET /events/{id}/attendance` — team view grouped by status
...
- [x] **T-006** `GET /events/{id}/attendance/me` — own response
...
- [x] **T-007** `PUT /events/{id}/attendance/me` — upsert own response
...
- [x] **T-008** `GET /users/me/abwesenheit` — list own rules
...
- [x] **T-009** `POST /users/me/abwesenheit` — create rule
  - **Acceptance:** Inserts rule; enqueues backfill job; returns `{ job_id }`; 400 on invalid payload
  - **Files:** `backend/src/main/kotlin/routes/AbwesenheitRoutes.kt`, `backend/src/main/kotlin/services/AbwesenheitService.kt`

- [x] **T-010** `PUT /users/me/abwesenheit/{ruleId}` — update rule
...
- [x] **T-011** `DELETE /users/me/abwesenheit/{ruleId}` — delete rule
  - **Acceptance:** Deletes rule; existing `declined-auto` attendance_responses unchanged; 404 if not owned by user
  - **Files:** `backend/src/main/kotlin/routes/AbwesenheitRoutes.kt`

- [x] **T-012** `GET /users/me/abwesenheit/backfill-status` — poll job status
...
- [x] **T-013** `GET /events/{id}/check-in` — full check-in list (coach only)
...
- [x] **T-014** `PUT /events/{id}/check-in/{userId}` — set check-in status
...
- [x] **T-017** Abwesenheit backfill job
...
- [x] **T-018** Post-event auto-present job
...
- [x] **T-019** Trigger backfill on event creation
...
- [x] **T-020** `AttendanceRepository` interface + impl
...
- [x] **T-021** `AbwesenheitRepository` interface + impl
...
- [x] **T-022** SQLDelight schema + offline mutation queue
...
- [x] **T-024** Deadline check in shared domain
...
- [x] **T-025** Client-side stats aggregation
...
- [x] **T-026** S1 Event Detail — My Attendance card
...
- [x] **T-027** S2 Begründung Sheet (bottom sheet)
...
- [x] **T-028** S3 Attendance List Screen
...
- [x] **T-030** S7 Add/Edit Absence Sheet
...
- [x] **T-031** S8 Player Statistics Screen
...
- [x] **T-032** S9 Team Statistics Screen
...
- [x] **T-034** Status badge component
