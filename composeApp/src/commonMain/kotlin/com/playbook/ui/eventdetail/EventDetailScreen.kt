...
import com.playbook.ui.attendance.BegrundungSheet
    ...
    LazyColumn(modifier = modifier.fillMaxSize().padding(horizontal = 16.dp)) {
        item {
            ...
            val dateStr = if (sameDay) {
                "${startLocal.dayOfMonth} ${startLocal.month.name.take(3)} ${startLocal.year}"
            } else {
                "${startLocal.dayOfWeek.name.take(3)} ${startLocal.dayOfMonth} ${startLocal.month.name.take(3)} – ${endLocal.dayOfWeek.name.take(3)} ${endLocal.dayOfMonth} ${endLocal.month.name.take(3)}"
            }
            val sH = startLocal.hour.toString().padStart(2, '0')
            val sM = startLocal.minute.toString().padStart(2, '0')
            val eH = endLocal.hour.toString().padStart(2, '0')
            val eM = endLocal.minute.toString().padStart(2, '0')
            val timeStr = "$sH:$sM – $eH:$eM"
            DetailRow(label = "Date", value = dateStr)
            DetailRow(label = "Time", value = timeStr)

            event.meetupAt?.let { meetup ->
                val meetupLocal = meetup.toLocalDateTime(tz)
                val mH = meetupLocal.hour.toString().padStart(2, '0')
                val mM = meetupLocal.minute.toString().padStart(2, '0')
                DetailRow(label = "Meet up", value = "$mH:$mM")
            }
